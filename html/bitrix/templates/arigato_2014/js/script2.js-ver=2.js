...
$("#order-nodelivery").change(function(){ 
  if($(this).attr("checked")){ 
    ...
    $("#order-date-label").html("Дата самовывоза");     
    $("#order-time-label").html("Время самовывоза");
  }else{ 
    $("#order-delivery-block").slideUp();
   $(".order-streetbox").slideDown();
   $("#order-date-label").html("Дата доставки");
   $("#order-time-label").html("Время доставки");
  } 
});
...
*/
    ...
    if (!pbc.hasClass('pb-pop-c-inactive')) {
            ...
            if (pbc.hasClass('pb-pop-c-inc-grey')) //для всплывающей корзины
            {
                ...
                $('#basket_all').empty();
                var textdiv="<div class='order-coast-text'>"+data["sum"]+" руб.</div><a href='http://www.arisushi.ru/order/' class='order-bottom'>оформить</a><div style='clear:both'>";
            ...
            else //для основного менб
            
            {
             //Всплывающее меню обработчик
    ...
    //китайские коробочки
	    if (params=="chinees")
        {
            ...
            if ((garnir=="none")||(garnir=="")) {alert ("Выберете гарнир");return false;}
            else 
               if ((sous=="none")||(sous=="")) {alert ("Выберете соус");return false;}
               
        }
        
    //китайские коробочки в окне    
	...
	if (!pbc.hasClass('pb-pop-c-inactive')) {
        ...
        if (!pbc.hasClass('pb-pop-c-dec-order')) //если не на странице оформления
        {            
			$('#basket_all').empty();
            var textdiv="<div class='order-coast-text'>"+data['sum']+" руб.</div><a href='http://www.arisushi.ru/order/' class='order-bottom'>оформить</a><div style='clear:both'>";
			$('#basket_all').append(textdiv);
         ...
         
         {
            ...
            $('#basket-popup').removeClass('active');
          	$('#basket_all').empty();
            var textdiv="<div class='empty-basket'>КОРЗИНА ПУСТА</div>";
			$('#basket_all').append(textdiv);
...
$('#basket_all').empty();
$('#basket_all').append("<div class='empty-basket'>КОРЗИНА ПУСТА</div>");
...
function newreview()
{
    ...
    showpopupwindow();
    data='<form method="POST" action="http://www.arisushi.ru/bitrix/templates/arigato_2014/js/company/reviews/send.php" id="review-form" onsubmit="submitreview(this);return false;">';
    data+='<textarea class="ovziv-item-add-text" placeholder="Помогите сделать Аригато еще лучше, напишите нам" name="text"></textarea>';
    data+='<input placeholder="Ваше имя" name="myname" class="ovziv-item-add-name" value="">';
    data+='<input type="submit" class="ovziv-item-add-bottom" value="ОТПРАВИТЬ">';
...
function action_form(a) {
      ...
      //msg)
      if (a.elements["telefon"].value=="") {alert ("Введите телефон (только цифры)");return;}
      if (ValidPhone(a.elements["telefon"].value)=="ERROR") {alert ("Номер телефона неверен");return;}
        ...
        $.ajax({
          ...
          success: function(data) {
            if (data["message"]=="OK") {popup_okno(600,210,a.elements["name"].value+", спасибо за заказ!<br/>Вы заказали «"+data["name"]+"»,<br/> стоимостью "+data["price"]+" руб.<br/>В течении 5 минут мы свяжемся с Вами для ­ уточнения данных Вашему по заказ",true);}
            if (data["message"]=="telefon") alert("Введите телефон");
            if (data["message"]=="id") alert("Товар не найден");
          },
