...

    {
        ...
        if($filter['alter']){
            ...
            foreach($filter['alter'] as $key => $val){
                if($not == true) {
                    $sql = "SELECT content.* FROM content, field_value where ". $where_not ." AND content.id = field_value.element_id AND field_value.value LIKE '%" . $val . "%' AND content.page_id = '" . $page . "' AND field_value.code = '" . $key . "' order by content.id DESC {$limit}";
                } else {
